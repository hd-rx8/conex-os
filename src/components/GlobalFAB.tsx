...
const GlobalFAB: React.FC = () => {
  ...
  const handleWorkspaceSubmit = async (data: any) => {
    setIsSubmitting(true);
    try {
      await modals.workspace.onSubmit(data);
      toast.success('Workspace criado com sucesso!');
    } catch (error: unknown) {
      toast.error(errorMessage(error, 'Erro ao criar workspace'));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* Workspace Modal */}
      {modals.workspace.isOpen && (
        <CreateWorkspaceModal
          isOpen={modals.workspace.isOpen}
          onClose={() => modals.workspace.setIsOpen(false)}
          onSuccess={() => toast.success('Workspace criado com sucesso!')}
        />
      )}
